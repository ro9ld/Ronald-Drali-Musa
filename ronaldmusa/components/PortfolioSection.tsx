...
function isStack(item: PortfolioItem) {
...
export default function PortfolioSection({ items }: Props) {
  ...
  const viewerLabel = currentItem
    ? isStack(currentItem)
      ? currentItem.photos![stackIndex]
      : currentItem.placeholderLabel
    : "";
  ...
  return (
    <>
      <div className="grid grid-cols-3 gap-[1.5px] sm:gap-[3px] lg:grid-cols-4">
        {visible.map((item, i) => (
